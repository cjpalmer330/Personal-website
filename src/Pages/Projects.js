  ...
  return (
    <div className="Project h-screen">
      ...
      <body className="overflow-hidden h-screen bg-slate-900 text-white font-mono">
        <div
        class="z-0">
          <div aria-hidden="true" className="transition-transform duration-1000 absolute    inset-y-24 inset-x-56 w-32 rounded-full rotate-45 bg-gradient-to-b from-pink-600    to-purple-600 blur-3xl">
          </div>
          <div aria-hidden="true" className="absolute inset-y-96 inset-x-[550px] w-32 h-  [500px]   rotate-45 rounded-full bg-gradient-to-b from-pink-600 to-purple-600   blur-3xl z-0">
          </div>
        </div>
        <div className="p-12 grid grid-rows-4 grid-cols-2 gap-16 text-center text-2xl opacity-90">
          <Link
            className="pt-2 pb-2 pl-4 pr-4 border shadow-2xl hover:scale-105 shadow-slate-200/40 rounded-3xl transition-all duration-300 bg-slate-700"
            to="/Projects/PersonalWebsite"
            >
            <h3 className="text-3xl font-bold font-sans text-slate-200 ">
              Portfolio
            </h3>
            <h1
